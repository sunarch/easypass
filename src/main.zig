...
pub fn main() !void {
    ...
    var invalid_arg_found: bool = false;
    var args_index: u8 = 0;
    for(args) |arg| {
        if (args_index == 0) {
            args_index += 1;
        ...
        // Check options: `if .. else if` from here
        ...
        else if (std.mem.eql(u8, arg, "-w") or
        ...
        else if (std.mem.eql(u8, arg, "-c6"))  { gen_options.count =  6; }
        else if (std.mem.eql(u8, arg, "-c7"))  { gen_options.count =  7; }
        else if (std.mem.eql(u8, arg, "-c8"))  { gen_options.count =  8; }
        else if (std.mem.eql(u8, arg, "-c9"))  { gen_options.count =  9; }
        else if (std.mem.eql(u8, arg, "-c10")) { gen_options.count = 10; }
        else if (std.mem.eql(u8, arg, "-c11")) { gen_options.count = 11; }
        else if (std.mem.eql(u8, arg, "-c12")) { gen_options.count = 12; }

        else if (std.mem.eql(u8, arg, "-wAGR"))    { try wordlist.set_wordlist(.wordlist_agr_en_original); }
        else if (std.mem.eql(u8, arg, "-wAGRalt")) { try wordlist.set_wordlist(.wordlist_agr_en_alt); }
        else if (std.mem.eql(u8, arg, "-wEFF"))    { try wordlist.set_wordlist(.wordlist_eff_large); }
        else if (std.mem.eql(u8, arg, "-wEFFs1"))  { try wordlist.set_wordlist(.wordlist_eff_short_1); }
        else if (std.mem.eql(u8, arg, "-wEFFs2"))  { try wordlist.set_wordlist(.wordlist_eff_short_2_0); }

        else {
            std.debug.print("Error: Unrecognized CLI argument: '{s}'\n", .{ arg });
